...
class JobRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
	...
	public function getJobsList($settings, $order, $search,$klinik) {		
		...
		foreach ($res as $key => $value) {
			...
			if($value['join_check']==1){
				$arr_1[] = $value;
			}
			if($value['ab_sofort']==1){
				$arr_2[] = $value;
			}
			if($value['text_check']==1){
				$arr_3[]= $value;
			}
			...
			$newArray = array_merge($arr_1, $arr_2, $arr_3);

			$_data = array();
			foreach ($newArray as $v) {
			  if (isset($_data[$v['uid']])) {
			    // found duplicate
			    continue;
			  }
			  $_data[$v['uid']] = $v;
			...
			$newArray1 = array_values($_data);

		//	$data['newData'] = $newArray1; 


		return $data;
	}
	
	public function getnewJobsList($settings, $order, $search,$klinik) {
		...
		foreach ($res as $key => $value) {
			...
			if($value['join_check']==1 && $value['ab_sofort']!=1){
				$arr_1[] = $value;
			}else if($value['ab_sofort']==1){
				$arr_2[] = $value;
			}else if($value['text_check']==1){
				$arr_3[]= $value;
			}
